   ...
   //load this document when the window loads
      ...
      let precipitation =[];
      let pressure = [];
      let exercise_type_value = [];
      let awake_pain_value = [];

      let data_date = [];

      let dateArray = [];  // add a new global variable to hold all the dates in the database
     ...
     //User data entry - enters the data the user inputs in the modal to the database
      ...
      button.addEventListener('click', async event => {
        ...
        exercise_type_value = document.getElementById('exercise_type').value;
        awake_pain_value = document.getElementById('awake_pain').value;
        ...
        function validateForm(){
          ...
          for (let i = 0; i < dateArray.length; i++){
            if (dateArray[i] == data_date){
              console.log(dateArray[i]);
              alert("You have entered data for this date already");
        ...
        data = {pain_value, sleep_value, sleep_value_mins, overall_sleep, exercise_value, 
          pressure: api_data, precipitation: precipitation, data_date, extra_info_value, exercise_type_value, awake_pain_value}
      ...
      async function chartIt() { //function that draws all the charts
        ...
        // fill the dataArray with all the dates in the database
        for (let i = 0; i < data.length; i++){
          dateArray[i] = data[i].data_date;
        }
      ...
      async function getData() {
        ...
        for (let i = 0; i<data.length; i++){
          ...
          awake_pain_value[i] = data[i].awake_pain_value;
          exercise_type_value[i] = data[i].exercise_type_value;
...
////// work out exercise donut chart ////////////////////////////////////////////////
          if (exercise_type_value[i] == "lightExercise") {
            lightExercise++;
          } 
          else if (exercise_type_value[i] == "moderateExercise") {
            moderateExercise++;
          }
          else {
            heavyExercise++;
...
// count days woke up with pain////////////////////
          if (awake_pain_value[i] =="yes"){
            woke_pain++;
          }
...
// to work out the numbers that get added together for the sum
        ...
        var p_average = psum / (headache_days + headache_free); //work out the average
...
//   //work out  exercise average from entered data ////////////////////////////////////////////////////////////////////////////////////////
        ...
        var e_average = esum / (Number(lightExercise) + Number(moderateExercise) + Number(heavyExercise));
...
//wake up with pain chart ///////////////////////////////////////////////////
const ctxWokePain = document.getElementById('woke_pain_chart').getContext('2d');
        
        const woke_pain_chart = new Chart(ctxWokePain, {
       
          type: 'pie',
          data: {
            labels: ["Woke Up With Pain", "Didn't Wake Up With Pain"],
            datasets: [{
              
              backgroundColor: ["#fec667", "#6abeb8"],
              data: [woke_pain, no_woke_pain],
            }]
          },
          options: {
            responsive: true,
...
/////////////// end of sleep chart ///////////////////////////////////////////////////////////////////////////////////////
      ...
      //api
      const api_url ="https://cors-anywhere.herokuapp.com/http://api.weatherunlocked.com/api/forecast/uk.GL526EG?app_id=52529cb0&app_key=408f3450df9671e205aa4bb265290472"
